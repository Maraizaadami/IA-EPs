...
#derivada da funçao de ativaçao
...
#Arquitetura da MLP
mlp.architecture <- function(input.length = 2, #tamanho das entradas, variaveis 
			     hidden.length = 2,#n de neuronios na camada escondida
			     output.length = 1, #n°de neuronios na camada de saida 
			     my.f = f,#função de ativiação
			     my.df_dnet = df_dnet) { #derivada da ativação
	#modelo
	layers = list()

	#geracao numero aleatório de pesos e tethas para camada escondida
	layers$hidden = matrix(runif(min=-0.5, max=0.5, 
			n=hidden.length*(input.length+1)), 
			       nrow=hidden.length, ncol=input.length+1)

	
	#geracao numero aleatório de pesos e tethas para camada de saída
...
#envia os dados : recebe um modelo e um padrão de entrada
mlp.forward <- function(model, x_p) {
	# x = c(1, 0)
	
	# camada escondida : calcula o f de net
...
#função de treino, vai da última camada para a primeira . Recebe um modelo, o conjunto de dados, o passo do treinamento ( gradiente descendente), limiar de parada)
mlp.backpropagation <- function(X, Y, model, eta=0.1, threshold=1e-2) {
	
	sqerror = 2 * threshold
	while (sqerror > threshold) { #enquanto erro maior que o limiar de parada
	    sqerror = 0
		...
		fwd = mlp.forward(model, x_p) #resultado
		o_p = fwd$f_o_net_o_pk #Valor obtido

		delta_p = y_p - o_p #erro o que espero - o que obtive
		...
		model$layers$hidden = model$layers$hidden +
	     ...
	     sqerror = sqerror / nrow(X) #soma dos erro dividido por linha
